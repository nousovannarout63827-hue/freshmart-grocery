...
<div style="padding: 24px; max-width: 700px; margin: 0 auto;">
    ...
    <div style="margin-bottom: 24px;">
        ...
        <h2 style="font-weight: 800; color: #1e293b; margin: 8px 0 0 0; font-size: 24px;">✏️ Edit Promo Code</h2>
        <p style="color: #64748b; margin: 4px 0 0 0; font-size: 14px;">Update the discount code settings.</p>
    </div>
    ...
    <!-- Form -->
    <form action="{{ route('admin.coupons.update', $coupon->id) }}" method="POST" style="background: white; border-radius: 16px; padding: 30px; border: 1px solid #f1f5f9; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
        @csrf
        @method('PUT')
        ...
        <div style="margin-bottom: 24px;">
            ...
            <input type="text" name="code" value="{{ old('code', $coupon->code) }}" placeholder="e.g. SUMMER20" required 
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 24px;">
            ...
            <div>
                ...
                <input type="number" name="value" step="0.01" min="0" max="999999.99" value="{{ old('value', $coupon->value) }}" 
        ...
        <div style="margin-bottom: 24px;">
            ...
            <input type="number" name="min_purchase" step="0.01" min="0" max="999999.99" value="{{ old('min_purchase', $coupon->min_purchase) }}" 
        ...
        <div style="margin-bottom: 30px;">
            ...
            <select name="status" required 
                    ...
                    onfocus="this.style.borderColor='#10b981'; this.style.boxShadow='0 0 0 3px rgba(16,185,129,0.1)';"
                    onblur="this.style.borderColor='#cbd5e1'; this.style.boxShadow='none';">
                <option value="1" {{ old('status', $coupon->status) == 1 ? 'selected' : '' }}>Active (Customers can use it)</option>
                <option value="0" {{ old('status', $coupon->status) == 0 ? 'selected' : '' }}>Disabled (Turned off)</option>
            </select>
        </div>
    ...
    <form action="{{ route('admin.coupons.destroy', $coupon->id) }}" method="POST" style="margin-top: 16px; text-align: center;" onsubmit="return confirm('Are you sure you want to delete coupon {{ $coupon->code }}? This cannot be undone.');">
        @csrf
        @method('DELETE')
        <button type="submit" style="background: transparent; color: #dc2626; border: 1px solid #dc2626; padding: 12px 24px; border-radius: 10px; font-size: 14px; font-weight: 700; cursor: pointer; transition: all 0.2s;"
                onmouseover="this.style.background='#dc2626'; this.style.color='white';"
                onmouseout="this.style.background='transparent'; this.style.color='#dc2626';">
            🗑️ Delete This Coupon
        </button>
    </form>
</div>
@endsection
